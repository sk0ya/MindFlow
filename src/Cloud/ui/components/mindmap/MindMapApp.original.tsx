...
import { exportMindMapAsJSON, importMindMapFromJSON } from '../../../core/storage/StorageManager.ts';
...
const MindMapApp: React.FC = () => {
  ...
  // Save handler reserved for future manual save functionality
  ...
  const handleShowCustomization = useCallback((_node: Node, position?: Position): void => {
  ...
  const handleShowImageModal = (image: any) => {
    setModalImage(image);
  ...
  const handleShowFileActionMenu = (file: any, nodeId: string, position: any) => {
  ...
  // Auth modal close handler reserved for future auth UI
  // const _handleCloseAuthModal = () => {
  ...
  const handleUserClick = (_user: any) => {
  ...
  // 競合処理関連
  // Conflict resolution handler reserved for future conflict UI
  // const _handleConflictResolved = (_conflict: any) => {
